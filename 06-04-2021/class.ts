...
class symetricEncryption implements IEncryption{
    encrypt(data: string):string{
        return `xyz$(data)`;
    }
    decrypt(data: string):string{
...
class asymetricEncryption implements IEncryption{
    encrypt(data: string):string{
        return `abc$(data)`;
    }
    decrypt(data: string):string{
...
class Oracle{
    ...
    save(data: string): void{
        let enccryptedData = this._encObj.encrypt(data);
        // save data to DB
        console
    }
}
...
let dbObj = new Oracle(new asymetricEncryption());
dbObj.save('Hello')
